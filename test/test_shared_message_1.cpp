...
class reference_counted_message : public etl::ireference_counted_message {
 public:
  ...
  [[nodiscard]] virtual etl::ireference_counter& get_reference_counter()
      override {
...
struct integral_limits<signed char> {
  static constexpr signed char min = (-0x7f - 1);
...
struct integral_limits<unsigned char> {
  ...
  static constexpr unsigned char max = (0x7f * 2 + 1);
  static constexpr int bits = 8;
  static constexpr bool is_signed = etl::is_signed<unsigned char>::value;
};
template <>
struct integral_limits<char> {
  static constexpr char min = (etl::is_signed<char>::value) ? (-0x7f - 1) : 0;
  static constexpr char max =
      (etl::is_signed<char>::value) ? 0x7f : static_cast<char>((0x7f * 2 + 1));
  static constexpr int bits = 8;
...
struct integral_limits<short> {
  static constexpr short min = (-0x7fff - 1);
  static constexpr short max = 0x7fff;
  static constexpr int bits = 8 * (sizeof(short) / sizeof(char));
...
struct integral_limits<unsigned short> {
  ...
  static constexpr unsigned short max = (0x7fff * 2 + 1);
  static constexpr int bits = 8 * (sizeof(unsigned short) / sizeof(char));
  static constexpr bool is_signed = etl::is_signed<unsigned short>::value;
};

template <>
struct integral_limits<int> {
  static constexpr int min = (-0x7fffffff - 1);
  static constexpr int max = 0x7fffffff;
  static constexpr int bits = 8 * (sizeof(int) / sizeof(char));
...
struct integral_limits<unsigned int> {
  ...
  static constexpr unsigned int max = (0x7fffffff * 2U + 1U);
  static constexpr int bits = 8 * (sizeof(unsigned int) / sizeof(char));
  static constexpr bool is_signed = etl::is_signed<unsigned int>::value;
};

template <>
struct integral_limits<long> {
  static constexpr long min = (-0x7fffffffffffffffL - 1L);
  static constexpr long max = 0x7fffffffffffffffL;
  static constexpr int bits = 8 * (sizeof(long) / sizeof(char));
...
struct integral_limits<unsigned long> {
  ...
  static constexpr unsigned long max = (0x7fffffffffffffffL * 2UL + 1UL);
  static constexpr int bits = 8 * (sizeof(unsigned long) / sizeof(char));
  static constexpr bool is_signed = etl::is_signed<unsigned long>::value;
};
template <>
struct integral_limits<long long> {
  static constexpr long long min = (-0x7fffffffffffffffLL - 1);
  static constexpr long long max = 0x7fffffffffffffffLL;
  static constexpr int bits = 8 * (sizeof(long long) / sizeof(char));
...
struct integral_limits<unsigned long long> {
  ...
  static constexpr unsigned long long max = (0x7fffffffffffffffLL * 2ULL + 1);
  static constexpr int bits = 8 * (sizeof(unsigned long long) / sizeof(char));
...
class smallest_type {
 ...
 public:
  using type = typename etl::conditional<(etl::size_of<T1>::value <
                                          etl::size_of<smallest_other>::value),
                                         T1,
                                         smallest_other>::type;
...
struct smallest_uint_for_value {
 private:
  static constexpr int TYPE_INDEX = ((VALUE > (255)) ? 1 : 0) +
                                    ((VALUE > (65535)) ? 1 : 0) +
                                    ((VALUE > (4294967295U)) ? 1 : 0);
...
struct smallest_int_for_value {
 private:
  static constexpr int TYPE_INDEX =
      (((VALUE > intmax_t((127))) || (VALUE < intmax_t((-128)))) ? 1 : 0) +
      (((VALUE > intmax_t((32767))) || (VALUE < intmax_t((-32767 - 1)))) ? 1
                                                                         : 0) +
      (((VALUE > intmax_t((2147483647))) ||
        (VALUE < intmax_t((-2147483647 - 1))))
           ? 1
           : 0);
...
struct largest_alignment {
  ...
  using type =
      typename etl::conditional<(etl::alignment_of<T1>::value >
...
class reference_counted_message_pool_release_failure
 ...
 public:
  reference_counted_message_pool_release_failure(string_type file_name_,
                                                 numeric_type line_number_)
      : reference_counted_message_pool_exception(
            ("reference_counted_message_pool:release failure"),
            file_name_,
            line_number_) {}
};
